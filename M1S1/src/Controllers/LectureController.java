...
public class LectureController {
    ...
    public void Update(Lecture data) throws Exception {
        con.getConnection();
        con.aud("UPDATE lecture SET name  = '" + data.getName() + "',emp_id  = '" + data.getEmp_id() + "',faculty  = '" + data.getFaculty() + "',level  = '" + data.getLevel() + "',department  = '" + data.getDepartment() + "',center  = '" + data.getCenter() + "',building  = '" + data.getBuilding() + "',rand  = '" + data.getRand() + "' WHERE id = '" + data.getId() + "'");
    }

    public void Delete(Lecture data) throws Exception {
        con.getConnection();
        con.aud("DELETE FROM lecture WHERE id = '" + data.getId() + "'");
    }

    public List<Lecture> SearchAll() throws Exception {
        List<Lecture> objList = new ArrayList<Lecture>();
        con.getConnection();
        ResultSet rset = con.srh("SELECT * FROM lecture");
        while (rset.next()) {
    ...
    public List<Lecture> Search(Lecture data) throws Exception {
        List<Lecture> objList = new ArrayList<Lecture>();
        con.getConnection();
        ResultSet rset = con.srh("SELECT * FROM lecture WHERE id = '" + data.getId() + "'");
        while (rset.next()) {
            Lecture obj = new Lecture();
            obj.setId(rset.getInt(1));
            obj.setName(rset.getString(2));
    ...
    public String getLastLecture() throws Exception {
        String value = "";
        con.getConnection();
        ResultSet rset = con.srh("SELECT * FROM lecture ORDER BY id DESC LIMIT 1;");
        if (rset.next()) {
